...
- **Releases:** the app's version and its What's new page come from `data/release_notes.json`. A user-visible change adds an entry at the top and bumps `pyproject.toml` to match, in the same commit (see `README.md`'s Releasing section); tests guard the file and the version sync. The how-to (`templates/help.html`) names buttons and fields exactly as the app shows them; when a change alters a label or the flow it describes, update it in the same change (tests check its labels against the app).
