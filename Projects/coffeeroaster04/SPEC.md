...
Each saved roast record shall store the id of the selected roast profile and a snapshot of the resolved (carry-forward-applied) target temperatures as of the time the roast was saved (and, since Phase 15, the profile's style).

### Profile styles **(Phase 15)**
A profile has a style: **drip** (filter; what every profile was before styles) or **espresso**. It is chosen on the page where the roaster is chosen, before the profile form, and can never change afterward: the targets are shaped for a style, so changing it would silently invalidate them. The edit form shows it read-only, and a submitted change is rejected (400). A profile or link with no style means drip; an unknown style is rejected.
Each saved roast records its profile's style (`profile_style`), like its roaster, so it stays self-describing if the profile is edited or deleted. At startup, once and idempotently, every profile and record with no style becomes drip (a record takes its profile's style); files are rewritten only when something changed.
The style is shown as a faint tag beside the roaster's tag wherever the roaster's tag shows (the profile lists, the Add roast profile picker, the Add Roast heading), on the profile form, and on the roast detail page. It is a separate element from the roaster tag, and is not a column in the roast list or CSV.
The profile wizard takes its values from the roaster's `wizard.espresso` block for an espresso profile (see "Roaster data (Tier 2)") and from the base block for drip; the wizard's script is unchanged, because the page config carries the same fields either way. A roaster with no espresso values has no espresso wizard. For espresso the wizard always says its values are estimates, for the calibrated roaster too, because no roaster has calibrated espresso data (the SR800's logged roasts are drip).
What the espresso values are: espresso develops longer than filter, and no more than that is claimed. One published guide puts filter development at 20 to 25% of the roast and espresso at "often 25 to 30 percent or more" (ICT Coffee, "Roasting for Filter vs. Espresso"); Scott Rao's 20 to 25% is for most roast levels and does not mention espresso; the sources found give no figure for time to first crack. So each roaster's espresso development-time ratios are its own drip ratios times 1.2 (rounded to two places, written out per roaster), and its espresso time to first crack and natural-process adjustment are its drip values, copied. All are listed as inferred, and are to be replaced by measured values as espresso roasts are logged; the wizard's curve, which ends at first crack, therefore differs from drip only in the longer target development time.
...
At startup, once, and idempotently: every profile with no roaster is assigned the Fresh Roast SR800; every record with no roaster takes its profile's roaster (the SR800 if the profile is gone) and every record without a `temp_unit` gets its roaster's unit. Files are rewritten only when something changed. The profile style is migrated the same way (see "Profile styles").
...
| `wizard` | `null` when there is no temperature readout, else `{time_to_first_crack_s: {low, medium, high}, natural_time_adjust_s, dtr_by_level, profile_start_temp, default_first_crack_temp}`, plus `espresso: {time_to_first_crack_s, natural_time_adjust_s, dtr_by_level}` (Phase 15): the values that depend on the profile style; the start and first-crack temperatures belong to the machine and are shared. Present exactly when `wizard` is. |
...
- Only the SR800 is calibrated, and only for drip; every other wizard value, and every `wizard.espresso` value (listed in `inferred` as `wizard.espresso`, with its source), is an estimate to be refined from logged roasts.
...
For each roaster and profile style with logged roasts (an espresso section is headed with the style, e.g. "Fresh Roast SR800 (Espresso)", and is compared with the roaster's espresso values, never the drip ones; espresso is never shown as calibrated; records with no roaster, or one no longer in `roasters.json`, are reported under their own headings without comparisons) it shows the roast count, whether the roaster is marked calibrated, and:
