...
Per-tier development time ratio (DTR = development time ⁄ total time) targets and the Maillard-phase pace constants (`MAILLARD_PACE`, `NATURAL_PACE_ADJUST`, `PROFILE_START_TEMP` in `profile_form.html`) were originally drawn from commonly published drum-roaster guidance, which turned out to run meaningfully slower than fluid-bed home roasters like the Fresh Roast machines this app targets — first-crack estimates around 8-9 minutes, versus the roughly 4-8 minute range reported for Fresh Roast machines by Sweet Maria's own SR540 tip sheet and corroborating sources. Both tables were rescaled by a uniform factor off one real logged Fresh Roast result (medium density, washed: 6:15 first crack, 1:10 development time, landed at Full City, i.e. ~15.7% DTR versus the original table's 21%), preserving each table's original relative shape (denser/natural beans still pace slower; darker roasts still carry a higher DTR) while anchoring the absolute scale to this machine. This is a rough first pass, not a validated model — only one real data point exists, for one density/process combination; the values should be revisited once more real roasts are logged across a range of density/process/roast-level combinations.
