...
Per-tier development time ratio (DTR = development time ⁄ total time) targets are approximate, drawn from commonly published roasting guidance (e.g. light roasts around a 15–18% DTR, dark roasts around a 28–31% DTR) rather than a single canonical source, since practice varies by roaster and roaster community; they're a starting point, not a guarantee.
The Maillard-phase pace and starting-temperature constants (`MAILLARD_PACE`, `PROFILE_START_TEMP` in `profile_form.html`) are likewise generic estimates, not derived from the user's own logged roasts; they are a known area for future recalibration once real roast history exists to check them against.
